...
with st.sidebar:
    ...
    api_key = st.text_input("APIキーを入力", type="password", help="Google AI StudioでAPIキーを取得してください")
    if api_key:
...
with tab3:
    ...
    if ratio < 50:
        remaining = required - net_cal
        st.warning(f"⚠️ カロリーが不足しています。あと {remaining} kcal 必要です。")
    elif ratio < 90:
        remaining = required - net_cal
        st.info(f"✅ もう少しで目標達成！あと {remaining} kcal です。")
    elif ratio <= 110:
        st.success("🎉 今日のカロリーは理想的です！")
    else:
        over = net_cal - required
        st.error(f"⚠️ {over} kcal オーバーです。運動で消費しましょう。")
    ...
    if today_records:
        for r in today_records:
            ...
            color = "red" if r["type"] == "meal" else "blue"
            st.markdown(f"""
